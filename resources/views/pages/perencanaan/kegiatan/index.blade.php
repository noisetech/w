            ...
            <div class="col-lg-3 col-md-3 col-sm-12 h-100">
                <div class="card">
                    <div class="card-body p-3">
                        ...
                        <div class="alert alert-{{ isset($active) && $active == 'kegiatan program' ? 'dark' : 'light' }}">
                            <strong
                                class="text-{{ isset($active) && $active == 'kegiatan program' ? 'white' : 'black' }}">Bidang
                            </strong>
                        </div>

                        <div class="alert alert-{{ isset($active) && $active == 'kegiatan program' ? 'dark' : 'light' }}">
                            <strong
                                class="text-{{ isset($active) && $active == 'kegiatan program' ? 'white' : 'black' }}">Program
                            </strong>
                        </div>
                        <div class="alert alert-{{ isset($active) && $active == 'kegiatan program' ? 'dark' : 'light' }}">
                            <strong
                                class="text-{{ isset($active) && $active == 'kegiatan program' ? 'white' : 'black' }}">Kegiatan</strong>
                        </div>
                ...
                <div class="card h-100">
                    ...
                    <div class="container">
                        <div id="form-create" style="display: none;">
                            <form action="#" method="post" id="form_tambah_kegiatan">
                                @csrf
...
@push('script')
    ...
    <script>
        $("#form_tambah_kegiatan").submit(function(e) {
            e.preventDefault();
            const fd = new FormData(this);
            $.ajax({
                url: '{{ route('perencanaan.p_form_tambah_kegiatan') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
        ...
        function formEditKegiatanById(data) {
            let id = $(data).data('id');

            $('.btn-add').hide();

            $.ajax({
                url: '{{ route('perencanaan.formEditKegiatanById') }}',
                method: 'get',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
        ...
        // form submit edit kegiatan
        $(document).on('submit', '#form_edit_kegiatan', function(e) {
            e.preventDefault();
            const fd = new FormData(this);
            $.ajax({
                url: '{{ route('perencanaan.p_formEditKegiatanById') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
