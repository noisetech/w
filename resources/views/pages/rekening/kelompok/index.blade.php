...
@section('title', 'Kelompok Rekening')
@section('content')
    {{ navbar('Kelompok Rekening', 'List Kelompok Rekening') }}
            ...
            <div class="col-lg-3 col-md-3 col-sm-12 h-100">
                <div class="card">
                    <div class="card-body p-3">
                        ...
                        <div class="alert alert-{{ isset($active) && $active == 'kelompok' ? 'dark' : 'light' }}">
                            <strong class="text-{{ isset($active) && $active == 'kelompok' ? 'white' : 'black' }}">Kelompok
                                Rekening
                            </strong>
                        </div>

                        <div class="alert alert-{{ isset($active) && $active == 'jenis' ? 'dark' : 'light' }}">
                            <strong class="text-{{ isset($active) && $active == 'jenis' ? 'white' : 'black' }}">Jenis
                                Rekening
                            </strong>
                        </div>
                        <div class="alert alert-{{ isset($active) && $active == 'objek' ? 'dark' : 'light' }}">
                            <strong class="text-{{ isset($active) && $active == 'objek' ? 'white' : 'black' }}">Objek
                                Rekening</strong>
                        </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-1">
                                        <div class="form-group">
                                            <input type="hidden" class="form-control" name="akun_rekening_id"
                                                value="{{ $akun->id }}">
                                        </div>
                                    </div>
                                </div>
...
@push('script')
    ...
    <script>
        $("#form_create").submit(function(e) {
            ...
            $.ajax({
                url: '{{ route('rekening.p_tambah_kelompok_rekening') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
        ...
        $(document).on('submit', '#form_edit', function(e) {
            ...
            const fd = new FormData(this);
            $.ajax({
                url: '{{ route('rekening.p_edit_kelompok_rekening') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
    ...
    <script>
        $('#dataTable').DataTable({
            ...
            order: [],
            ajax: {
                url: "{{ route('rekening.data_kelompok_rekening') }}",
                data: {
                    id: "{{ encrypsi($segment) }}"
                },
            },
            columns: [{
                    ...
                    render: function(data, type, row) {
                        return '<a href="/dashboard/rekening/jenis/' + encrypsi(row.id) + '" >' + data +
                            '</a>';
                    }
                },
                {
    ...
    <script>
        ...
        function edit(data) {
            ...
            $.ajax({
                url: '{{ route('rekening.edit_kelompok_rekening') }}',
                method: 'get',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
        ...
        function hapus(data) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('rekening.hapus_kelompok_rekening') }}",
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },
