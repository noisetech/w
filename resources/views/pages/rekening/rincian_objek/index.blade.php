...
@section('title', 'Rincian Objek Rekening')
@section('content')
    {{ navbar('Rincian Objek Rekening', 'List Rincian Objek Rekening') }}
                                ...
                                <div class="row">
                                    <div class="col-sm-1">
                                        <div class="form-group">
                                            <input type="hidden" class="form-control" name="objek_rekening_id"
                                                value="{{ $objek->id }}">
                                        </div>
                                    </div>
                                </div>
...
<script>
        $("#form_create").submit(function(e) {
            ...
            $.ajax({
                url: '{{ route('rekening.p_tambah_rincian_objek_rekening') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
        ...
        $(document).on('submit', '#form_edit', function(e) {
            ...
            const fd = new FormData(this);
            $.ajax({
                url: '{{ route('rekening.p_edit_rincian_objek_rekening') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
    ...
    <script>
        $('#dataTable').DataTable({
            ...
            order: [],
            ajax: {
                url: "{{ route('rekening.data_rincian_objek_rekening') }}",
                data: {
                    id: "{{ encrypsi($segment) }}"
                },
            },
    ...
    <script>
        ...
        function edit(data) {
            ...
            $.ajax({
                url: '{{ route('rekening.edit_rincian_objek_rekening') }}',
                method: 'get',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
        ...
        function hapus(data) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('rekening.hapus_rincian_objek_rekening') }}",
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },
