            ...
            <div class="col-lg-3 col-md-3 col-sm-12 h-100">
                <div class="card">
                    <div class="card-body p-3">
                        ...
                        <div class="alert alert-{{ isset($active) && $active == 'unit' ? 'dark' : 'light' }}">
                            <strong class="text-{{ isset($active) && $active == 'unit' ? 'white' : 'black' }}">Bidang
                            </strong>
                        </div>
                        <div class="alert alert-{{ isset($active) && $active == 'unit' ? 'dark' : 'light' }}">
                            <strong
                                class="text-{{ isset($active) && $active == 'unit' ? 'white' : 'black' }}">Organisasi</strong>
                        </div>
                        <div class="alert alert-{{ isset($active) && $active == 'unit' ? 'dark' : 'light' }}">
                            <strong class="text-{{ isset($active) && $active == 'unit' ? 'white' : 'black' }}">Unit</strong>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card h-100">
                    ...
                    <div class="container">
                        <div id="form-create" style="display: none;">
                            <form action="#" method="post" id="form_tambah_unit">
                                @csrf
...
@push('script')
    ...
    <script>
        $('#dataTable').DataTable({
            ...
            order: [],
            ajax: {
                url: "{{ route('perencanaan_organisasi.data_unit') }}",
                data: {
                    id: "{{ encrypsi($segment) }}"
                },
            },
    ...
    <script>
        $("#form_tambah_unit").submit(function(e) {
            ...
            $.ajax({
                url: '{{ route('perencanaan_organisasi.p_formTambahUnit') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
        ...
        $(document).on('submit', '#form_edit_unit', function(e) {
            ...
            const fd = new FormData(this);
            $.ajax({
                url: '{{ route('perencanaan_organisasi.p_formEditUnitgById') }}',
                method: 'post',
                data: fd,
                cache: false,
                processData: false,
        ...
        function hapusUnit(data) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('perencanaan_organisasi.hapusUnit') }}",
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function formEditUnitById(data) {
            let id = $(data).data('id');

            $('.btn-add').hide();

            $.ajax({
                url: '{{ route('perencanaan_organisasi.formEditUnitById') }}',
                method: 'get',
                data: {
                    id: id,
                    _token: '{{ csrf_token() }}'
